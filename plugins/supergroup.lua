...
local function callback_kicked(cb_extra, success, result)
	--vardump(result)
	local text = "کاربران مسدود برای سوپرگروه "..cb_extra.receiver.."\n\n"
	local i = 1
	for k,v in pairsByKeys(result) do
		if not v.print_name then
			name = " "
...
local function unlock_group_username(msg, data, target)
  ...
  if group_username_lock == 'no' then
    return '🔹قفل یوزرنیم غیر فعال بوده است'
  else
...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return '✅ قفل ارسال لینک در فعال بوده است'
  else
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return '✅ قفل ارسال لینک غیرفعال بوده است'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return '✅ قفل ارسال لینک غیرفعال شد'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if not is_owner(msg) then
    return "این دستور فقط برای مدیر گروه است"
  end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return '✅ اسپم غیرفعال است . پیام های طولانی پاک نخواهند شد'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return '✅ قفل اسپم غیرفعال شد . پیام های طولانی پاک نخواهند شد'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return 'قفل پیام ورود و خروج فعال نیست'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل پیام ورود و خروج غیرفعال شد'
  end
end
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'پاک کردن ربات' and is_admin1(msg) and not matches[2] then
